...
func ChunkCode(parentNodeID, source, language string, lang *sitter.Language, headingFn func(*sitter.Node, []byte) string, cfg Config) ([]Chunk, error) {
	...
	for i := uint(0); i < root.ChildCount(); i++ {
		...
		case "function_declaration", "method_declaration", "method_definition", "class_declaration":
			c := extractCodeChunk(parentNodeID, sourceBytes, lines, node, &chunkIndex, chunkType(name), language, cfg, headingFn)
			if c != nil {
		...
		case "type_declaration":
			...
			for j := uint(0); j < node.ChildCount(); j++ {
				...
				if child.GrammarName() == "type_spec" {
					c := extractCodeChunk(parentNodeID, sourceBytes, lines, child, &chunkIndex, ChunkTypeCodeStruct, language, cfg, headingFn)
					if c != nil {
						chunks = append(chunks, *c)
					}
				}
