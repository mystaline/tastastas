...
func TestStage_ReadFilteringHidesMismatchedEdge(t *testing.T) {
	...
	// Insert a stage-mismatched edge directly via raw SQL, bypassing the
	// UpsertEdge invariant, to simulate a pre-existing violation.
	if _, err := s.db.ExecContext(ctx,
		`INSERT INTO edges (from_id, to_id, edge_type, confidence, confidence_tier, bidirectional) VALUES (?, ?, 'calls', 1.0, 'INFERRED', 0)`,
		a.ID, b.ID); err != nil {
		t.Fatal(err)
	}
	// Legacy cross-project edge (both unscoped, different projects) — must remain visible.
	if _, err := s.db.ExecContext(ctx,
		`INSERT INTO edges (from_id, to_id, edge_type, confidence, confidence_tier, bidirectional) VALUES (?, ?, 'calls', 1.0, 'INFERRED', 0)`,
		legacy1.ID, legacy2.ID); err != nil {
		t.Fatal(err)
	}
	// Edge with dangling endpoint — must remain visible (LEFT JOIN).
	if _, err := s.db.ExecContext(ctx,
		`INSERT INTO edges (from_id, to_id, edge_type, confidence, confidence_tier, bidirectional) VALUES (?, ?, 'calls', 1.0, 'INFERRED', 0)`,
	...
	// ListEdgesByProject requires both endpoints to exist (inner join by
	// design), so only the legacy cross-project edge is visible here —
	// the dangling-endpoint edge is correctly excluded.
